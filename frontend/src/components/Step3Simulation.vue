...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // 从Step2传入的最大rodadas数
  minutesPerRound: {
    type: Number,
    default: 30 // Padrão每rodadas30分钟
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: 未开始, 1: 运行中, 2: Concluído
...
// 格式化模拟流逝Hora（根据rodadas次和每rodadas分钟数计算）
...
const doStartSimulation = async () => {
  ...
  // 先Redefinir所有Status，确保不会受到上一次模拟的影响
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('正在启动Simulação paralela em duas plataformas...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // 强制重新开始
      enable_graph_memory_update: true  // 开启动态Grafo更新
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Configurações最大模拟rodadas数: ${props.maxRounds}`)
    }
    
    addLog('已开启动态Grafo更新模式')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 已清理旧的模拟日志，重新Iniciar Simulação')
      }
      addLog('✓ 模拟引擎启动Sucesso')
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测模拟是否Concluído（通过 runner_status 或平台完成Status判断）
    ...
    console.warn('获取运行Status失败:', err)
  }
}

// 检查所有启用的平台是否Concluído
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 不自动滚动，让用户自由VisualizarHora轴
      // 新动作会在底部追加
    }
  } catch (err) {
    console.warn('获取详细Status失败:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Geração de Relatório请求已Enviar，Por favor, aguarde...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('正在启动Geração de Relatório...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Geração de Relatório任务已启动: ${reportId}`)
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ 启动Geração de Relatório失败: ${res.error || '未知Erro'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ 启动Geração de Relatório异常: ${err.message}`)
    isGeneratingReport.value = false
  }
}
