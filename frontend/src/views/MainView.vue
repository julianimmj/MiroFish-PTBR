...
// Step State
const currentStep = ref(1) // 1: Construção do Grafo, 2: Configuração do Ambiente, 3: Iniciar Simulação, 4: Geração de Relatório, 5: Interação Profunda
...
const handleNextStep = (params = {}) => {
  if (currentStep.value < 5) {
    ...
    // 如果是从 Step 2 Entrando na Etapa 3，记录模拟rodadas数配置
...
const handleGoBack = () => {
  if (currentStep.value > 1) {
    currentStep.value--
    addLog(`Retornando à Etapa ${currentStep.value}: ${stepNames[currentStep.value - 1]}`)
  }
}
