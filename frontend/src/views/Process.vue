      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        ...
        <div class="project-panel">
          ...
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item">
              <span class="item-label">Requisito de Simulação</span>
...
// 处理新建Projeto - 调用 ontology/generate API
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = 'Projeto初始化失败: ' + (err.message || '未知Erro')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || '加载Projeto失败'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = '加载Projeto失败: ' + (err.message || '未知Erro')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: '正在启动Construção do Grafo...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Construção do Grafo任务已启动...'
      
      // Salvar task_id 用于rodadas询
      const taskId = response.data.task_id
      
      // 启动Grafo数据rodadas询（独立于任务Statusrodadas询）
      startGraphPolling()
      
      // 启动任务Statusrodadas询
      startPollingTask(taskId)
    } else {
      error.value = response.error || '启动Construção do Grafo失败'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = '启动Construção do Grafo失败: ' + (err.message || '未知Erro')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Construção do Grafo完成，Carregando完整数据...')
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 加载完整Grafo:', projectResponse.data.graph_id)
            await loadGraph(projectResponse.data.graph_id)
            console.log('✅ Grafo加载完成')
          }
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Construção do Grafo失败: ' + (task.error || '未知Erro')
        buildProgress.value = null
      }
    }
  } catch (err) {
